...
const char *DEV_NAME = "synergy-input";
...
int InitUinput() {
	...
	ioctl(_fd, UI_SET_ABSBIT, ABS_X) == -1 ||
	ioctl(_fd, UI_SET_ABSBIT, ABS_Y) == -1 ||
...
int WriteToInput(uint16_t type, uint16_t code, int32_t value) {
...
int WriteToInputSyn(uint16_t type, uint16_t code, int32_t value) {
...
int MouseLeftDown() {
	return WriteToInputSyn(EV_KEY, BTN_LEFT, DOWN);
}

int MouseLeftUp() {
	return WriteToInputSyn(EV_KEY, BTN_LEFT, UP);
}

int MouseRightDown() {
	return WriteToInputSyn(EV_KEY, BTN_RIGHT, DOWN);
}
